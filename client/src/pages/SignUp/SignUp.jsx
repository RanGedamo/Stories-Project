...
function SignUp() {
  ...
  const submitAvatar = async () => {
    ...
    await axios.post('https://api.cloudinary.com/v1_1/dcapisw77/image/upload', formData)
      ...
      }).catch(err => setErrInput("image not found"))
  };

  const submitUser = async (data) => {
    return await disptch(register(data)).then(res => console.log(res))
      .catch(error =>{ 
        ...
        <MDBRow className='d-flex justify-content-center align-items-center h-100'>
          <MDBCol col='12'>
            <MDBCard className='bg-dark  text-white my-5 mx-auto' style={{ borderRadius: '1rem', maxWidth: '400px' }}>
             ...
             {errInput?<div style={{color:"red",fontSize:25,fontFamily:"-moz-initial",border:"1px red solid" ,width:"100%"}} >{errInput}</div>:""}
